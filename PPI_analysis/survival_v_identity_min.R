...
get.data <- function(this.folder, which.chain) {
  ...
  for(i in dirs) {
    ...
    cutoff.surv <- min(divergence[!survived])
    cutoff.divergence <- max(divergence[survived & divergence>=cutoff.surv])
...
pdf(paste('~/Sandbox/marcotte/complex_divergence_simul/figures/survival_v_divergence_min_', this.chain, '.pdf', sep=''), height=11, width=12)
