...
get.data <- function(this.folder, which.chain) {
  ...
  for(i in dirs) {
    ...
    cutoff.divergence <- 1 - max(dat$interface_identity[which(dat$ancestral_interaction > survival.value)])
    
    cutoff.count[is.infinite(cutoff.count) | is.na(cutoff.count)] <- max(dat$count)
    cutoff.divergence[is.infinite(cutoff.divergence) | is.na(cutoff.divergence)] <- max(1 - dat$interface_identity)
    
    survival.divergence <- append(survival.divergence, cutoff.divergence)
    status.divergence <- append(status.divergence, as.numeric(!cutoff.divergence == max(1 - dat$interface_identity)))
...
pdf(paste('~/Sandbox/complex_divergence_simul/figures/survival_v_interface_divergence_', this.chain, '.pdf', sep=''), height=11, width=12)
